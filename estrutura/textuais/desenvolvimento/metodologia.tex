...
A seguir, cada etapa será descrita de forma detalhada como foi realizada, quais ferramentas foram utilizadas e os problemas encontrados durante o processo.  
...
O filtro é utilizado com a finalidade de corrigir boa parte do ruído e pequenos erros de medição de distância gerado nos dados, provenientes da movimentação do robô enquanto coleta.


\section{Reconstrução}
\label{sec:reconstrucao}

A reconstrução do modelo tridimensional é gerada a partir da nuvem de pontos. Como exemplo, a Figura \ref{fig:reconstruction} mostra a reconstrução de um objeto passo a passo. Nessa parte do processo, já é obtido a forma final da superfície.
